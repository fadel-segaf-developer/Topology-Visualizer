...
async function fetchOptional(apiPath, params = {}, options = {}) {
  try {
    return await fetchPaged(apiPath, params, options);
  } catch (error) {
    console.warn(`Skipping ${apiPath}: ${error.message.split("\n")[0]}`);
    return [];
  }
}
...
async function exportRepository() {
  ...
  const repoInfo = await fetchJson(`${BASE}/api/v1/repos/${OWNER}/${REPO}`);
  await writeJson(path.join(outDir, "repo.json"), repoInfo);

  const [issues, pulls, milestones, labels, releases, tags, branches, collaborators, projects] = await Promise.all([
    ...
    fetchOptional(`/repos/${OWNER}/${REPO}/collaborators`),
    fetchOptional(`/repos/${OWNER}/${REPO}/projects`)
  ]);
...
async function exportIssueComments(issues) {
  ...
  for (const issue of issues) {
    ...
    const comments = await fetchOptional(`/repos/${OWNER}/${REPO}/issues/${number}/comments`);
...
async function exportPullArtifacts(pulls) {
  ...
  for (const pull of pulls) {
    ...
    const [comments, reviews] = await Promise.all([
      fetchOptional(`/repos/${OWNER}/${REPO}/pulls/${number}/comments`),
      fetchOptional(`/repos/${OWNER}/${REPO}/pulls/${number}/reviews`)
    ]);
